  ...
  <div class="admin-page">
    ...
    <form class="form" @submit.prevent="save">
      ...
      <div v-for="entry in selectedSpecs" :key="entry.id" class="spec-block">
        ...
        <div
          v-if="getGroup(entry.id).photos.length"
          class="image-preview photos-grid"
        >
          <p>Попередній перегляд фото:</p>
          <div
            v-for="photo in getGroup(entry.id).photos"
            :key="photo.uid"
            class="photo-item"
          >
            <img
              :src="photo.preview"
              :alt="`Фото випуску ${entry.specialty}`"
            />
            <button
              type="button"
              class="btn tiny danger"
              @click="removeSpecPhoto(entry.id, photo.uid)"
            >
              Прибрати
            </button>
          </div>
        ...
        <div class="form-row">
          <h3>
            ...
            {{ entry.studyForm || "форма не вибрана" }})
          </h3>
      ...
      <div class="buttons">
        ...
        <button type="button" class="btn" @click="resetForm">Скасувати</button>
      </div>

      <p v-if="error" class="error">{{ error }}</p>
      <p v-if="success" class="success">Випуск створено</p>
    </form>
  </div>
</template>
...
const specialtyTags = ref([]); // всі доступні спец-ті з бази
const selectedSpecs = ref([]); // [{ id, specialty, studyForm, _prevStudyForm }]
const newSpecialtyTag = ref("");
const newSpecSection = ref(""); // форма для нової спец-ті
...
/* додати існуючу спец-ть зі списку */
...
/* локальне видалення однієї спеціальності */
...
/* ---------- структури по спеціальностях (групи) ---------- */
...
/* гарантовано повертає групу для entryId */
function getGroup(entryId) {
  const groups = specialtyGroups.value;
  if (!groups[entryId]) {
    groups[entryId] = {
      photos: [], // [{ uid, file, preview }]
...
function onSpecPhotosChange(entryId, e) {
  ...
  for (const file of files) {
    ...
    g.photos.push({ uid, file, preview });
  }

  // щоб можна було ще раз вибрати ті ж файли
  target.value = "";
}

function removeSpecPhoto(entryId, photoUid) {
  const g = getGroup(entryId);
  const idx = g.photos.findIndex((p) => p.uid === photoUid);
  if (idx === -1) return;

  const [photo] = g.photos.splice(idx, 1);
  if (photo.preview) URL.revokeObjectURL(photo.preview);
}

/* вантажимо всі фото, повертаємо масив images */
async function uploadSpecialtyPhotos() {
  ...
  for (const entry of selectedSpecs.value) {
    ...
    for (const photo of g.photos) {
      ...
      // formData.append("category", "graduate"); // можна залишити або видалити, бек це ігнорує
...
function resetForm() {
  title.value = "";
  newSpecialtyTag.value = "";
  newSpecSection.value = "";
  selectedSpecs.value = [];
  specialtyGroups.value = {};
  success.value = false;
  error.value = "";
}
...
async function save() {
  ...
  // id — глобально унікальний у межах випуску,
  // index — локальний для кожної спеціальності
  ...
  for (const entry of selectedSpecs.value) {
    ...
    for (const s of g.students) {
      ...
      preparedStudents.push({
        ...
        name: s.name.trim(),
        specialty: entry.specialty,
        section: entry.studyForm, // може бути й "Немає"
        year: yearNum,
  ...
  try {
    ...
    if (status === 409) {
      error.value = msgFromServer || "Випуск з таким роком уже існує";
    } else {
      error.value = msgFromServer || "Помилка при створенні випуску";
    }
  } finally {
    saving.value = false;
  }
