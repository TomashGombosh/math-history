#!/usr/bin/env node
/**
 * Loads migration/data/teachers-seed.json into DynamoDB (BatchWrite + META SEQ#TEACHER).
 * Usage: node load-teachers-dynamodb.mjs <jsonPath> <tableName> <endpointUrl?>
 */
...
if (!jsonPath || !tableName) {
  console.error("Usage: node load-teachers-dynamodb.mjs <teachers-seed.json> <tableName> [endpointUrl]");
  process.exit(1);
}

const raw = JSON.parse(readFileSync(jsonPath, "utf8"));
const items = raw.items;
const teacherSeq = Number(raw.teacherSeq);
if (!Array.isArray(items) || !Number.isFinite(teacherSeq)) {
  console.error("Invalid seed file: expected { items, teacherSeq }");
  process.exit(1);
}
...
async function main() {
  ...
  await putMetaSeq();
  console.error(`Done. META SEQ#TEACHER seq=${teacherSeq}`);
}
