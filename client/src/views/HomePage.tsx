...
const HOME_FAQ: FaqItem[] = [
  ...
  {
    question:
      "Як знайти інформацію про конкретного викладача математичного факультету УжНУ?",
    answer:
...
export default function HomePage() {
  ...
  useEffect(() => {
    let cancelled = false;
    void Promise.all([
      apiGet<TeachersCursorResponse>("/api/teachers", {
        cursor: 1,
        limit: 12,
      }).then((r) => setTeachers(pickTeachers(r.teachers))),
  ...
  return (
    <div className="home">
      ...
      <section className="home-block intro-section">
        ...
        <p>
          Цей сайт створений для збереження та представлення історії
          математичного факультету Ужгородського національного університету. Тут
          зібрано інформацію про викладачів, які формували факультет у різні
          роки, займалися науковою діяльністю, навчали студентів та зробили
          вагомий внесок у розвиток математичної освіти на Закарпатті.
        </p>
        <p>
          ...
          простежити розвиток факультету, кількісні зміни та успішність
          студентів у різні періоди.
        </p>
        <address className="home-org-address">
          <span lang="uk">
            Кафедра математики, Ужгородський національний університет
          </span>
          {" — "}
      ...
      {loading ? (
        ...
        <>
          <section className="home-block">
            ...
            {!teachers.length ? (
              <p className="home-empty-hint">Немає даних про викладачів.</p>
            ) : null}
          ...
          <section className="home-block">
            ...
            <div className="years-grid">
              {years.map((item) => (
                <Link
                  key={item.year}
                  to={ROUTES.graduatesYear(item.year)}
                  className="year-card"
                >
            ...
            {!years.length ? (
              <p className="home-empty-hint">Немає даних про роки випуску.</p>
            ) : null}
            <div className="home-footer-link">
              <Link to={ROUTES.graduates} className="home-link">
